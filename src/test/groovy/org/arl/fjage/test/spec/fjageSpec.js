...
describe("A Gateway", function () {
  var foo = 1;
  it("should be able to be consuctured", function (done) {
    var gw;
  ...
  it("constructor should cache Gateways to the same url+port", function (done) {
    const gw = new Gateway();
  ...
  it("registers itself with the global window.fjage object", function (done) {
    var gw = new Gateway();
  ...
  it("shoudl remove itself from global array when closed", function (done) {
    const gw = new Gateway();
    setTimeout(() => {
      console.log("closing");
      gw.close();
