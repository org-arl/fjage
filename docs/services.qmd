...
A client interested in availing a specific service can look for an agent that provides the service, and then interact with it using the alternate syntax described in the [Messaging](messages.qmd#alternate-syntax) chapter:
...
::: {.callout-note}
As long as the agents are added to the container before starting the platform, fjåge guarantees that all agents are initialized before any agent behaviors are called.
:::
