...
Here `WebServer.addStatic()` serves the static web resources bundled in the fjåge jar, and `openWebSocketServer()` opens a WebSocket endpoint (at `ws://hostname:8080/ws`) that fjage.js gateways connect to. `WebServer.start()` starts serving once everything is set up, and returns `false` if the web server cannot start (for example, if the port is already in use). If you just want to try things out, running `./fjage.sh -web` in the fjåge repository sets up an equivalent web-enabled container for you.
