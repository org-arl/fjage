...
Once we have developed our agents, it is easy to deploy them on multiple nodes as necessary. To do so, we require one [MasterContainer](https://org-arl.github.io/fjage/javadoc/index.html?org/arl/fjage/remote/MasterContainer.html) in our application, and any number of [SlaveContainer](https://org-arl.github.io/fjage/javadoc/index.html?org/arl/fjage/remote/SlaveContainer.html). The master container must be started first, and the slave containers connect to it.
...
It is often useful to connect a [console shell](shell.qmd) to a running fjåge application to monitor, interrogate or modify it. To do this, we ensure that the application is running in a master container (and possible some slave containers). We then create a `rconsole.sh`:
...
Only agents may access messaging and related functionality provided by fjåge. For example, non-agent Java or Groovy threads cannot send messages to, or receive messages. To aid interaction of such threads with agents, fjåge provides a [Gateway](https://org-arl.github.io/fjage/javadoc/index.html?org/arl/fjage/remote/Gateway.html) class. This class provides agent-like functionality to non-agent threads by creating a proxy agent in a slave container that has access to this functionality. Using the `Gateway` is fairly simple:

``` groovy
Gateway gw = new Gateway(hostname, port)
