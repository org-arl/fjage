...
fjåge provides a **lightweight** and **easy-to-learn** platform for [agent-oriented software development](https://en.wikipedia.org/wiki/Agent-oriented_programming) in Java and [Groovy](https://groovy-lang.org/).

## Why fjåge?

Several frameworks exist for agent-oriented software development. For Java programmers, [JADE](https://jade.tilab.com/) provides a [FIPA](https://www.fipa.org/)-compliant framework for multi-agent systems. The [API](https://org-arl.github.io/fjage/javadoc/) for fjåge is largely based on the API available in JADE, and so any developer familiar with JADE should have very little difficulty learning to develop using fjåge. However, there are some significant differences between the philosophy between the two projects. The key advantages of fjåge are:
...
- fjåge has APIs for access from Java, Groovy, Python, C, Julia and Javascript applications, and a well-defined JSON protocol that can be used to develop connectors from any other language.
...
- **DiscreteEventSimulator** -- This platform allows agents to be tested in a [discrete event simulation](https://en.wikipedia.org/wiki/Discrete_event_simulation) mode. Essentially, all the time-related functions and scheduling use a *virtual time*. The passage of virtual time is simulated such that computation and processing does not take any virtual time, while scheduling requests are met accurately in virtual time. The virtual time advances in discrete steps such that the time when no agent is active is effectively skipped. This potentially allows for simulation of hours of virtual time within seconds.
...
fjåge is released under the open source [simplified (3-clause) BSD license](https://github.com/org-arl/fjage/blob/master/LICENSE).

## Availability

fjåge is available as a binary release via the [GitHub Maven repository](https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-apache-maven-registry):
...
From version 2.1.0 onwards, fjåge is published in the GitHub Maven repository only. Previous versions are still available in Maven Central.

Its source code is available via [GitHub](https://github.com/org-arl/fjage):
