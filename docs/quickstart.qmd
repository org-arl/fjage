...
We also assume a UNIX style platform (Linux, Mac OS X or [Cygwin](https://www.cygwin.com/) on Windows) with Java Development Kit (JDK) 1.8 or higher installed. If you don't have one, you'll need to [install](https://www.oracle.com/java/technologies/downloads/) one. We also assume that you have network connectivity to download the relevant files automatically.

Create a new folder for your first fjåge project. Lets call it `MyFjageProject`. Open a terminal window in this folder and download the [fjage_quickstart.sh](https://raw.githubusercontent.com/org-arl/fjage/master/docs/fjage_quickstart.sh) script in that folder:

``` sh
curl -O https://raw.githubusercontent.com/org-arl/fjage/master/docs/fjage_quickstart.sh
```
...
build/libs/
  ...
  cloning-1.9.12.jar
  gson-2.13.2.jar
  ... and other supporting jars (jetty, websocket, etc.)
etc/
  initrc.groovy
...
To run the agent, start fjåge and run the script by typing `run 'hello'`. This will return you to the interactive shell prompt. To check that your agent is indeed running, type `ps`. You may then shutdown fjåge as before and check the log file for your output:

```
bash$ ./fjage.sh
> run 'hello'
> ps
hello: HelloWorldAgent
shell: org.arl.fjage.shell.ShellAgent
> shutdown

bash$ cat logs/log-0.txt | grep HelloWorldAgent@
1783666307909|INFO|HelloWorldAgent@20:println|Hello world!!!
bash$
```
...
This is useful for testing. However, in a production system, you usually want to define agents in their own files, compile them and package them into a jar on the classpath. To do this, you would create a source file `HelloWorldAgent.groovy` (or `HelloWorldAgent.java`) with the class definition:
...
This code enables Groovy extensions in fjåge to add syntactic sugar for ease of writing Groovy agents, and then sequentially executes every initialization Groovy script given on the command line. In our case, this causes the `etc/initrc.groovy` to be executed. A simplified version of that script looks like this:
